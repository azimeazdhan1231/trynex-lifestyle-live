...
export default function CartModalFixed({ isOpen, onClose }: CartModalProps) {
  const { cart, updateItemQuantity, removeFromCart, clearCart, getTotalPrice, isLoaded } = useCart();
  ...
  // Use cart directly from the hook
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    <div className="bg-gradient-to-r from-emerald-50 to-green-50 border border-emerald-200 rounded-xl p-4 sm:p-5">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-sm font-medium text-gray-900">{getTotalPrice().totalItems} টি</span>
                      </div>
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-xl sm:text-2xl font-bold text-emerald-600 flex items-center">
                          {formatPrice(getTotalPrice().totalPrice)}
                    ...
                    <Button
                      ...
                      size="lg"
                      disabled={cart.length === 0 || getTotalPrice().totalItems === 0}
                      data-testid="button-checkout"
                    >
                      <ShoppingCart className="w-5 h-5 mr-2" />
                      চেকআউট করুন ({getTotalPrice().totalItems} টি পণ্য)
                    </Button>
                  </div>
                </div>
              );
